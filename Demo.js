...
async function bfs(button){
  if(!graph){
    console.log("Select a Graph from 'Get Example graph or add some nodes'")
    return
  }
...
async function birdsView(){
    const fitContentUrl=CYREST_BASE_URL+"/v1/apply/fit/"+network_SUID
    await fetch(fitContentUrl, {
    method: 'GET',
    headers: {
    ...
    imagelement.setAttribute("src",imageUrl)
    imagelement.setAttribute("width",width)
    imagelement.setAttribute("height",height)
    const birdViewContainer=document.getElementById("container")
    birdViewContainer.appendChild(imagelement)
}